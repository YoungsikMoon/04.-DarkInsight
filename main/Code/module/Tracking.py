...
class Tracking:
    ...
    # model : Yolo 모델
    def __init__(self, model, isUltra=False):
        self.model = model
    ...
    def track_video(self, obj_thres, update_thres, tracking_count, video_path, isSample=False, save_path=""):
        ...
        try: # 중간에 중단되도 메모리 해제하기 위함
            ...
            isPoseDetected = dict() # 눕는 포즈 감지 여부
            ...
            while True:
                ...
                for i in range(len(p_center)):
                    ...
                    # # 쓰러짐 이후 움직임 감지(40프레임) 및 상태 해제
                    # if isSpeedOver[min_ps] and len(ps_route[min_ps]) > 43:
                        
                    #     # 40프레임 이전 좌표정보
                    #     before_xy_40frame = ps_route[min_ps][len(ps_route[min_ps])-41]
                        
                    #     # 40프레임 이전 좌표정보와의 거리
                    #     dist_40frame = self.dist(before_xy_5frame, p_c[0], p_c[1])

                    #     # 거리가 정규화된 범위보다 커지면
                    #     if dist_40frame >= obj_thres/p_c[2]*100:
                    #         isSpeedOver[min_ps] = False # 쓰러짐 상태 해제
                    ...
                    if min_ps in isSpeedOver.keys() and isSpeedOver[min_ps][0]:
                        ...
                        # 쓰러짐 경보 출력
                        cv2.putText(img, f"!!FallDown Alarm!!", (p_xy[0]-100, p_xy[1]-40), cv2.FONT_HERSHEY_PLAIN, 5, (0,0,255), 3, cv2.LINE_AA) 
                    else:
